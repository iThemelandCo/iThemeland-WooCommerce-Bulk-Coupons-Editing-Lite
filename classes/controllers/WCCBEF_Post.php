...
class WCCBEF_Post
{
    ...
    private function __construct()
    {
        ...
        add_action('admin_post_wccbef_activation_plugin', [$this, 'activation_plugin']);
    }
    ...
    public function activation_plugin()
    {
        ...
        if (isset($_POST['activation_type'])) {
            if ($_POST['activation_type'] == 'skip') {
                update_option('wccbef_is_active', 'skipped');
                ...
                if (!empty($_POST['email']) && !empty($_POST['industry'])) {
                    ...
                    $info = $activation_service->activation([
                        ...
                        'product_id' => 'wccbef',
                        'product_name' => WCCBEF_LABEL,
                        'industry' => sanitize_text_field($_POST['industry']),
                        'multi_site' => is_multisite(),
                        'core_version' => null,
                        'subsystem_version' => WCCBEF_VERSION,
                    ]);

                    if (!empty($info) && is_array($info)) {
                        if (!empty($info['result']) && $info['result'] == true) {
                            update_option('wccbef_is_active', 'yes');
                            $message = esc_html__('Success !', WCCBEF_NAME);
                        } else {
                            update_option('wccbef_is_active', 'no');
                            $message = (!empty($info['message'])) ? esc_html($info['message']) : esc_html__('System Error !', WCCBEF_NAME);
                        }
                    } else {
                        update_option('wccbef_is_active', 'no');
                        $message = esc_html__('Connection Timeout! Please Try Again', WCCBEF_NAME);
                    }
                }
            }
        }
    ...
    public function meta_fields()
    {
        ...
        $this->redirect('meta-fields', esc_html__('Success !', WCCBEF_NAME));
    }
    ...
    public function column_manager_new_preset()
    {
        ...
        $this->redirect('column-manager', esc_html__('Success !', WCCBEF_NAME));
    }
    ...
    public function settings()
    {
        ...
        $this->redirect('settings', esc_html__('Success !', WCCBEF_NAME));
    }
    ...
    public function import_coupons()
    {
        ...
        $this->redirect('import-export', esc_html__($message, WCCBEF_NAME));
    }
